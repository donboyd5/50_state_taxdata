...
xub <- rep(50, inputs2$n_variables)
a <- proc.time()
sse_opt2 <- optimx::optimr(par=inputs2$x0, # inputs2$x0  sse_opt2$par
                 fn = eval_f_wfs,
                 gr = eval_grad_f_wfs,
                 # lower = xlb, upper = xub,
                 method = "BFGS", # L-BFGS-B Rcgmin BFGS
...
opts_ipopt <- list("print_level" = 0,
             ...
             "output_file" = here::here("out", "fn3all_os1_wadj.out"))

# setwd(here::here("temp1"))
#getwd()
wfs3 <- ipoptr(x0 = wadjust$xadj, # inputs2$x0 wfs1$solution, sse_opt$par sse_opt2$par
               lb = inputs2$xlb,
               ub = inputs2$xub,
...
eval_f_wfs(inputs2$x0, inputs2)
eval_f_wfs(wfs1$solution, inputs2)
eval_f_wfs(wfs2$solution, inputs2)
eval_f_wfs(wfs3$solution, inputs2)

cw <- check_weights(wfs3$solution)
cw %>% ht
sse_weights(cw)
cw %>%
  ggplot(aes(weight_total, weight)) +
...
wcheck <- inputs2$weight_totals %>%
  mutate(#weight1=eval_g_addup(wfs1$solution, inputs2),
         weight2=eval_g_addup(wfs3$solution, inputs2))
wcheck

wcheck %>%
  summarise(#sse1=sum((weight1 - weight_total)^2),
            sse2=sum((weight2 - weight_total)^2))
