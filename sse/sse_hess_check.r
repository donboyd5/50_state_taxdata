...
inputs <- get_inputs_sse(targets_df, iweights, ofe_sparse, target_scaling=TRUE, scale_goal=100, scale_type="max")

# investigate hessian ----
system.time(hess <- hessian(func=eval_f_sse, x=inputs$x0, inputs=inputs))
hess[1:10, 1:10] %>% round(1)
hess2 <- round(hess, 2)
isSymmetric(hess2)

set.seed(1234)
x_alt <- rnorm(length(inputs$x0))
system.time(hess_alt <- hessian(func=eval_f_sse, x=x_alt, inputs=inputs))

hess[1:10, 1:10] %>% round(1)
hess_alt[1:10, 1:10] %>% round(1)

sum(hess2==0) / length(hess2)

hess2 <- hess %>% round(3)
rownames(hess2) <- ofe_dense$xname
colnames(hess2) <- ofe_dense$xname
...
ofe_dense %>%
  filter(j<=10) %>%
  mutate(across(c(pop_nnz, pincp_sum), ~ . / iweight_state)) %>%
  mutate(h=iweight_state^2 * 2 * (pop_nnz^2 + pincp_sum^2)) # this gets the diagonal

hess2[1:10, 1:10]

# now try to get the off-diagonal elements
# create a data frame of ids where states are the same but pids differ
pidvec <- unique(ofe_dense$pid)

pid_stub <- ofe_dense %>%
  select(index=j, pidname=xname, pid, stabbr)

h1 <- pid_stub %>%
  select(i=index, ipid=pid, ist=stabbr, iname=pidname)
h2 <- tibble(jpid=pidvec)

hstub <- expand_grid(h1, h2)
h3 <- hstub %>%
  mutate(jname=paste0("p", jpid, "_", ist)) %>%
  left_join(pid_stub %>% 
              select(j=index, jname=pidname, ist=stabbr), 
            by = c("ist", "jname"))
ht(h3)

h4 <- h3 %>%
  left_join(ofe_dense %>% 
              select(iname=xname, iweight_state, all_of(target_vars)) %>%
              mutate(across(c(pop_nnz, pincp_sum), ~ . / iweight_state)),
            by="iname") %>%
  left_join(ofe_dense %>% 
              select(jname=xname, iweight_state, all_of(target_vars)) %>%
              mutate(across(c(pop_nnz, pincp_sum), ~ . / iweight_state)),
            by="jname",
            suffix=c(".i", ".j"))
ht(h4)

piv_vars <- c(paste0(target_vars, ".i"), paste0(target_vars, ".j"))
h5 <- h4 %>%
  pivot_longer(all_of(piv_vars)) %>%
  separate(name, c("varcalc", "ij"), sep="\\.")  %>%
  mutate(ij=paste0(ij, "val")) %>%
  pivot_wider(names_from = ij) %>%
  mutate(value=2 * iweight_state.i * iweight_state.j * ival * jval)

h6 <- h5 %>%
  group_by(i, j, iname, jname) %>%
  summarise(hvalue=sum(value), .groups = "drop") %>%
  arrange(i, j)
...
fhess <- function(x, obj_factor, hessian_lambda, inputs){
  return(obj_factor * hess)
}

hstruc <- llply(1:39, function(x) 1:39)
...
opts_ipopt <- list("print_level" = 0,
                   ...
                   "obj_scaling_factor" = 1e-1, # 1e-3, # default 1; 1e-1 pretty fast to feasible but not to optimal
                   # "nlp_scaling_max_gradient" = 100, # default is 100 - seems good
                   "output_file" = here::here("out", "test_hess3.out"))

# setwd(here::here("temp1"))
#getwd()

ip1 <- ipoptr(x0 = inputs$x0, # inputs$x0 wadjust$xadj sse_opt$par
              lb = inputs$xlb,
              ub = inputs$xub,
              ...
              eval_h = fhess, # the hessian is essential for this problem eval_h_xtop eval_h_xm1sq
              eval_h_structure = hstruc,
